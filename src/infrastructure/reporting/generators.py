...
负责将已经渲染好的日报 HTML 转换为图片，不生成其它报告格式或处理上传。
"""
    ...
    async def render_html_content_to_image(
        ...
        async with self._render_semaphore:
            try:
                ...
                logger.error("日报图片渲染调用失败: %s", e)
                return None

        if not image_data:
            logger.error("日报图片渲染返回空结果")
            return None

        if isinstance(image_data, bytes):
            if not self._is_valid_image_head(image_data[:10]):
                logger.error("日报图片渲染返回了无效图片字节")
                return None
        ...
        if isinstance(image_data, str):
            ...
            if not image_path.exists():
                logger.error("日报图片渲染返回了不存在的图片路径: %s", image_data)
                return None

            try:
                data_head = image_path.read_bytes()[:10]
            except Exception as e:
                logger.error("读取日报图片临时文件失败: %s", e)
                return None

            if not self._is_valid_image_head(data_head):
                logger.error("日报图片临时文件不是有效图片: %s", image_data)
                return None
            return image_data

        logger.error("日报图片渲染返回了不支持的数据类型: %s", type(image_data))
        return None
