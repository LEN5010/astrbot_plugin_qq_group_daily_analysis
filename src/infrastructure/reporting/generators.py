...
class ReportGenerator(IReportGenerator):
    ...
    async def generate_image_report(
        ...
        try:
            ...
            image_url = await self.render_html_content_to_image(
                html_content, group_id, html_render_func
            )
            ...
            for image_options in render_strategies:
                try:
                    ...
                    is_valid = bool(
                        actual_data_head
                        and (
                            actual_data_head.startswith(b"\xff\xd8")
    ...
    async def save_rendered_html_report(
        self,
        html_content: str,
        group_id: str,
    ) -> str | None:
        """
        将任意已渲染好的 HTML 内容保存到 HTML 输出目录。
        """
        if not html_content:
            return None
        ...
        try:
            ...
            logger.info(f"自定义 HTML 报告已保存: {html_path}")
            return str(html_path.absolute())
        except Exception as e:
            logger.error(f"保存自定义 HTML 报告失败: {e}", exc_info=True)
            return None
